...
- **See what a sync actually did** — a run opens into a per-object report rather than four totals: which objects were created, which fields changed on which object (and whether the change matters — a site entering the danger list is not a reworded description), which came through untouched, which the source stopped listing, and where the source now disagrees with a curator's edit. The report defaults to the changes worth a look and hides only minor field edits
