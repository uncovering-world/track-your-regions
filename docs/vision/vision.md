...
- **Only what a person has passed** — some sources are trusted outright; others, ones more prone to importer mistakes, are held back until a curator has looked. Once a source is gated, anything it wrote that nobody has checked yet is simply absent — no card, no marker, no line in a count — the same as if it did not exist. This applies at whatever size the gate catches a mistake: a whole new venue, one of its locations, or one work inside an otherwise-visible museum can each be held back on its own, without hiding the rest of what that source already offers
