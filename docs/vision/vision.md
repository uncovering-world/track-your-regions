...
Replace binary visited/not-visited with a spectrum of connection depth. First phase: visit-based states (Stranger, Passed Through, Explored, Deep Connection) derived from experience tracking, checklist input, and visit history. Second phase: add the Aware state (knowledgeable but never visited) once the quiz system can detect theoretical knowledge. Checklist items have three eligibility types: universal (apply everywhere), geographic (auto-detected from spatial data — coastal, mountainous, desert regions), and curated (hand-picked per region). See [`CONNECTION-LEVEL-CHECKLIST.md`](CONNECTION-LEVEL-CHECKLIST.md).
