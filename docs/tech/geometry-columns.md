...
## `regions` table (10 geometry columns)
...
| `display_geom` | MultiPolygon | **User-editable display geometry** for archipelagos. Allows manual override of how a region looks on the map. |
...
| `display_geom_3857` | MultiPolygon | `display_geom` | Full-resolution display geometry in Web Mercator. |
| `geom_simplified_low` | MultiPolygon | Best of: `ts_hull_geom_3857` > `display_geom_3857` > `geom_3857` | **Zoom 0-4**. Simplified with 5km tolerance, no smoothing. |
...
Zoom 9+:   ts_hull_geom_3857 or display_geom_3857 (archipelagos), else geom_3857
```

---

## `administrative_divisions` table (9 geometry columns)
...
| `display_geom` | MultiPolygon | Optional display override (rarely used for admin divisions). |
...
| Martin vector tiles (zoom 9+) | regions | `ts_hull_geom_3857`/`display_geom_3857` (archipelagos), else `geom_3857` |
