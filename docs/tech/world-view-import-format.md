...
| `sourceUrl` | string | No | Absolute http(s) URL of the original source page (e.g., Wikivoyage article); refused otherwise |
...
- **`sourceUrl`** — stored in `region_import_state.source_url`. Displayed as a link in the match review tree. Used to identify duplicate instances of the same region across the tree (for the "Sync to other instances" feature). Refused at upload unless it is an absolute http(s) URL — it is the one field of a node that becomes a link, where a `javascript:` value would run on click — and every surface that offers the page asks `safeHref` (`frontend/src/utils/safeHref.ts`) first, so a page the rule refuses is not linked (#703).
...
- **`regionMapUrl`** — stored in `region_import_state.region_map_url`. Shown as a reference image alongside GADM boundaries in the preview dialog. Also available as an overlay in the Custom Subregions dialog's Map View. Refused at upload unless it is an absolute http(s) URL, and drawn only from Wikimedia Commons, the one host the app trusts for pictures and where every map imported so far lives (`Special:FilePath/…` gets sized with `?width=`; a file on `upload.wikimedia.org` is asked for through the same door, and only under `/wikipedia/commons/`). A map hosted anywhere else, a Commons file that is not a picture — a `.pdf` map — or the `/wiki/File:…` page about a file rather than the file (what Commons' address bar gives; use the `Special:FilePath/…` form) is stored but never shown (#694, ADR-0043) — and never read: the CV colour match refuses it before it loads the region's divisions, with an error naming the map, and the admin image proxy answers such an address 400 (#706). A map from another host therefore makes the region one to match by hand, not one the pipeline will work on.
