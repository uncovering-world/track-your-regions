...
**In Docker** the package arrives as a named build context (`additional_contexts` in `docker-compose.yml`) copied to `/packages/shared`, where the `file:../packages/shared` link resolves from `/app` exactly as on a checkout; the dev stack mounts its `src/` over that copy on both containers. `packages/shared/README.md` has the same rules from the package's side.
