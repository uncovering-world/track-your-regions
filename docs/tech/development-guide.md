...
Request schemas are on `zod/v4`, as the response schemas are. The route files move onto the registry one by one: `/api/divisions`, `/api/geocode`, `/api/experiences`, `/api/world-views`, `/api/users`, `/api/ai` and `/health` are declared, and the rest still list their middleware by hand, with `validate()` and the guards that hold them (the Cache-Control lint rules, `cacheOverrides.test.ts`, `adminClientPaths.test.ts`). #793 removes those with the last file.
