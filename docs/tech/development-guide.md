...
- **SSE endpoints name `token` in their Zod query schema.** `EventSource` can't send headers, so JWT is passed as `?token=...`. On a declared route `requireAuth` reads it before the schema parses, and the schema names it so the documented parameters are the ones sent; on a route not yet declared, `validate()` replaces `req.query` and strips undeclared fields, so the schema must include `token: z.string().optional()` or the token is gone before auth.
...
- `access` (`public`, `optional`, `signed-in`, `curator`, `admin`) and `cache` (`no-store`, `revalidate`, `shared-revalidate`, `token`) are required. `token` is for an answer that hands out an access token — `no-store` with the `Pragma` RFC 6749 § 5.1 asks for — and `routerOf` requires it of any route whose response schema declares `accessToken`. `shared-revalidate`, the one a shared cache may keep, is only for a `public` route.
...
- The chain is the same for every route: limiter, the caller, the cache header, params, query, body, scope, handler. A refused caller gets its 401 or 403 before any 400, and a handler never runs on input its schemas refused.
- The handler takes `{ params, query, body, caller }`, typed from the schemas, and returns the body, which the registry sends through `respond()`. It returns `NO_CONTENT` for a 204 where the declaration says `noContent: true`, and a route that only ever answers 204 declares `response: NO_BODY`. A create declares `status: 201`. A server-sent stream declares `response: stream(EventSchema)`: the registry opens it, the handler writes each event with the `send` it is handed, and the registry ends it when the handler returns. A redirect the handler writes itself declares `response: REDIRECT`; where middleware of its own writes it (passport), the handler waits on `whenAnswered(res, next => …)`, which settles when the response finishes, since passport never calls `next` after a redirect. A `public` route's handler gets no `caller`, so a handler that shapes its answer by who asks does not compile there.
- `scope` names the world view an answer belongs to, read from the parsed input (`{ regionId }` or `{ worldViewId }`, or `undefined` for an optional filter). The registry answers 404 to anyone but an admin where it is hidden, after the body and before the handler.
- A handler's spec answers a request on its declared route with `answer(routeAt(routes, path), req, res)` from `backend/src/api/routeTesting.ts`: the route's own schemas parse the request, and what the handler throws answers as the error handler would.
- A handler fails by throwing: `notFound`, `badRequest` and `createError` from `middleware/errorHandler.ts`, or `failure(sentence, status, code?)` for a sentence the product wrote, which stays readable at a 5xx in production. A refusal whose answer carries more than its sentence — a stale card's 409 with the row as it now stands, for the client to redraw from — is thrown as `new Refusal(status, body)`, which the error handler sends as written and does not log as a failure; its `error` is a sentence the product wrote, and no key is an error's own text.
- `routerOf` refuses, at startup, a route an earlier one shadows (`/:id` above `/search`).

Request schemas are on `zod/v4`, as the response schemas are. The route files move onto the registry one by one: every route file but `/api/admin` is declared, and that one still lists its middleware by hand, with `validate()` and the guards that hold it (the Cache-Control lint rules, `cacheOverrides.test.ts`, `adminClientPaths.test.ts`). #793 removes those with the last file.
