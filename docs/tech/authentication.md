...
1. **Access Token**: Short-lived (15 minutes), sent in `Authorization: Bearer <token>` header, kept in memory only. Exception: SSE endpoints (`EventSource`) pass JWT as `token` query parameter since `EventSource` can't send custom headers. The `requireAuth` middleware checks both `Authorization` header and `req.query.token`. A declared stream's `requireAuth` reads the token before the route's schema parses the query, and the schema names `token` so the parameters it documents are the ones sent; on a route not yet declared, `validate()` replaces `req.query` with the parsed object, so its schema must name `token` or the token is gone before `requireAuth` runs
...
11. **Cache headers follow the middleware**: the middleware that puts an identity on the request is the one that says how the response may be cached. `requireAuth` marks every response it fronts `Cache-Control: private, no-store` + `Vary: Authorization` — the caller's own data, kept out of the browser's disk cache after sign-out (#710); `optionalAuth` marks its responses `private, no-cache` — public data shaped by the caller, which a shared cache may not serve to the next reader while the browser keeps its revalidation round-trip (#597). A handler that needs another value sets it after the middleware, and keeps the `private` unless it means to publish, since replacing the header drops it. One override does mean to: the admin image proxy answers `public, max-age=86400`, because what it returns is a Wikimedia Commons picture unchanged. A route declared `public` (ADR-0071) is the other case: it reads no token, so its `shared-revalidate` policy answers `public, no-cache`, and the registry refuses that policy on any route that reads one. Every other response keeps `private`. The reads whose body is public reference data behind the auth gate — the GADM geometry read `GET /api/divisions/:divisionId/geometry` and the geoshape proxy `GET /api/admin/wv-import/geoshape/:wikidataId` — say `private, no-cache`, which is the override an admin read most often needs: gated for who asks, not for what it returns. The geometry read's route declares it (`cache: 'revalidate'`, ADR-0071); the geoshape proxy, not yet declared, says it through `markPublicReferenceBody` (`backend/src/middleware/cacheHeaders.ts`). The SSE streams — `GET /api/admin/wv-import/matches/:worldViewId/color-match-stream`, `GET /api/admin/wv-import/matches/:worldViewId/coverage-stream` and `GET /api/world-views/regions/:regionId/geometry/compute-stream` — say `private, no-cache` too: the compute stream by its route's declared `revalidate` policy (ADR-0071), the two import streams through `markStreamBody` in the same module; and their token rides in the query string (`EventSource` cannot send headers), so nothing else holds a shared cache off them — the same for the admin images loaded as `<img src>` (`GET /api/admin/wv-import/water-crop/:reviewId/:componentId/:subCluster`, `GET /api/admin/wv-import/cluster-preview/:reviewId` and `GET /api/admin/wv-import/cluster-highlight/:reviewId/:label`), which take the token the same way and keep `private, max-age=300`. The responses handing out an access token say `private, no-store` for themselves, with the `Pragma: no-cache` RFC 6749 § 5.1 asks for beside it — the `token` cache policy their routes declare (`backend/src/api/route.ts`, ADR-0071), which `routerOf` requires of any route whose response schema declares `accessToken`: `POST /api/auth/login`, `/verify-email`, `/refresh` and `/exchange-code` run before any of this, and `POST /api/auth/change-password` is behind `requireAuth` and declares it too, so all of them read alike. The reasoning is in `docs/security/SECURITY.md` § Headers.
...
4. Declare its two routes in `authRoutes.ts`, each answering `REDIRECT`, with handlers beside Google's and Apple's in `controllers/auth/oauthController.ts` — a handler hands the request to passport inside `whenAnswered`, since passport answers with a redirect and never calls `next`:
   ```typescript
   defineRoute({
     method: 'get', path: '/{provider}', access: 'public', cache: 'no-store',
     response: REDIRECT,
     handler: startProvider,
   }),
   ```
