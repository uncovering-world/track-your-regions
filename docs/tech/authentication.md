...
11. **Cache headers follow the middleware**: the middleware that puts an identity on the request is the one that says how the response may be cached. `requireAuth` marks every response it fronts `Cache-Control: private, no-store` + `Vary: Authorization` — the caller's own data, kept out of the browser's disk cache after sign-out (#710); `optionalAuth` marks its responses `private, no-cache` — public data shaped by the caller, which a shared cache may not serve to the next reader while the browser keeps its revalidation round-trip (#597). A handler that needs another value sets it after the middleware, and keeps the `private` unless it means to publish, since replacing the header drops it. One override does mean to: the admin image proxy answers `public, max-age=86400`, because what it returns is a Wikimedia Commons picture unchanged. Every other one keeps it. The four reads whose body is public reference data behind the auth gate — the three GADM geometry reads and the geoshape proxy — say `private, no-cache` through `markPublicReferenceBody` (`backend/src/middleware/cacheHeaders.ts`), which is the override an admin read most often needs: gated for who asks, not for what it returns. The three SSE streams say `private, no-cache` too, through `markStreamBody` in the same module, and their token rides in the query string (`EventSource` cannot send headers), so nothing else holds a shared cache off them — the same for the three admin images loaded as `<img src>`, which take the token the same way and keep `private, max-age=300`. The responses handing out an access token say `private, no-store` for themselves, with the `Pragma: no-cache` RFC 6749 § 5.1 asks for beside it (`markTokenResponse`, `backend/src/middleware/cacheHeaders.ts`): four of the five run before any of this, and the fifth — `POST /api/auth/change-password` — is behind `requireAuth` and already carries the value, marked anyway so the five read alike. The reasoning is in `docs/security/SECURITY.md` § Headers.
