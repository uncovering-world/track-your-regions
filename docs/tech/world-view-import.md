...
| POST | `/matches/:worldViewId/finalize` | Close review — appends `'_done'` to current `source_type`; requires `skeleton_confirmed`, all work units signed off, and zero active global gaps (active = total − dismissed) |
...
3. **Match review** (`WorldViewImportReview.tsx`) — two view modes:
   ...
   - **Close Review** button — finalizes the match review, appending `'_done'` to the current `source_type` (e.g., `'wikivoyage'` → `'wikivoyage_done'`, `'imported'` → `'imported_done'`). Requires both: (1) no blocking match issues and (2) a passing coverage check (0 active uncovered GADM divisions — dismissed don't count). The server-side finalize gate also enforces: skeleton confirmed, all work units signed off, and zero active global gaps (active = total − dismissed) — returns 400 with an explicit message listing every unmet blocker. The world view remains fully editable from the WorldView Editor but no longer appears in the active review list
...
A focused per-country editing environment replacing the need to hunt inside the full import tree. Clicking a country row in the dashboard (or choosing "Open workspace" from its menu) navigates here. The persistent map shows child-region colored fills, a dashed reference-outline (from `referenceDivisionIds`), and orange overlap fills — gap fills (red, click-to-assign) and overlap fills render below the reference outline via MapLibre `beforeId`. Container rows in the tree display coverage % chips (both area-based and geoshape-based). The preview-comparison suite covers four modes: single-division preview, union preview (all clean suggestions vs region map/geoshape, with parent-map fallback), transfer preview (three-layer donor/moving/target dialog, per-row only when `wikidataId` is present), and view-map comparison of assigned divisions. Assignment finders (Geocode, DB Search, AI, Geoshape, Point Match) now report their results with a toast notification, an inline status line on the selected node, and a loud "none found — try X" prompt when no candidates are returned. Accepted candidates land in a **Proposed** bucket below the Assigned list, each annotated with source-method chips (e.g. "geoshape", "AI") and Accept-all / Dismiss-all batch controls, keeping proposed candidates visually distinct from already-assigned divisions. Proposed candidates also render live on the map as an amber fill with a dashed outline so the operator can see each shape before committing; clicking an amber shape on the map (or clicking its row in the Proposed list) accepts that single candidate, deletes the suggestion row, and replaces the amber overlay with a solid child-region fill. The Proposed list supports multi-select (checkboxes + select-all) with batch Accept selected / Accept + reject rest / Reject selected; a mixed selection accepts the clean divisions directly and routes the conflicted ones through the transfer preview.

Coverage gaps found by the checks bar now appear in a **Coverage Gaps panel** (rendered in the left column so the map stays visible) with per-gap focus-on-map and assign-to-suggested-region controls, plus a **Create region** action (Plan 4g) that turns a gap into a new region under a chosen parent (default the unit root) via `approveCoverageSuggestion(… 'create_region' …)`; confirming an assignment or creation immediately removes the gap from both the panel and the map and marks the checks stale so the admin knows to re-run. The panel lists the **highest-level uncovered divisions (gap boundaries)** — if an entire province is uncovered it shows the one province, not its districts/communes — so assigning a single boundary division covers its whole subtree in one action. (`analyzeCoverageGaps` shares the `getCoverageBoundaries` query with `verifyWorkUnit`, so the panel count always equals the checks-bar gap count.) The map paints uncovered divisions red, and clicking a red gap shape focuses its row in the panel and offers the geo-suggested assignment.
