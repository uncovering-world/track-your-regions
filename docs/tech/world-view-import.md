...
Coverage gaps found by the checks bar now appear in a **Coverage Gaps panel** (toggled from the gaps chip) with per-gap focus-on-map and assign-to-suggested-region controls; confirming an assignment immediately removes the gap from both the panel and the map and marks the checks stale so the admin knows to re-run. The map paints uncovered divisions red, and clicking a red gap shape focuses its row in the panel and offers the geo-suggested assignment.
