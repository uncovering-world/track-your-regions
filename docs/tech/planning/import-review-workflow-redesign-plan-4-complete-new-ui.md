...
**Status (2026-06-15):** sub-plans **4a–4i all DONE** (each has its own `…-plan-4{a..i}-*.md` doc + committed implementation + review): 4a write-side suggestion dedup, 4b dashboard global-gaps resolution, 4c multi-select batch accept/reject, 4d AI tools (suggest-children apply + review drawer), 4e map-image picker, 4f Re-match/Compute on the dashboard, 4g create-region-from-gap, 4h correctness (finalize zero-active-gaps server-side, sync matched-status gate, antimeridian bbox, mixed-unit map fill), 4i docs (world-view-import.md updated throughout + vision.md). **Legacy deletion + cutover ADR remain a separate future branch** (legacy retained as `?wvReview=` fallback). Outstanding before the import-review PR: `npm run security:all`.
