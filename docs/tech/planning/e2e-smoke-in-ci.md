...
`db/init/01-schema.sql` already seeds the `GADM` default world view (id 1) and
all three experience categories, so the fixture adds only content:

- at least one named root region under world view 1, with simple valid
