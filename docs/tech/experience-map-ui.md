...
- The world layer: `frontend/src/components/WorldExperiencePoints.tsx` — what Map Mode draws
  before a region is chosen, from a tile source rather than from a region's read
  (§ The world layer, below)
...
Map Mode uses four GeoJSON sources:

- `exp-extent`: the outline of one place — whichever is hovered, else whichever is selected — where the catalogue holds one, which today means an Archaeology site's excavation boundary as OpenStreetMap drew it (ADR-0059). Mounted **first**, so its 15% wash and its 2 px outline are added before every marker layer and nothing the reader clicks is painted over; the outline takes the object's own pin colour, carried on the feature. Set on selection, and on a hover that has *rested* — 150 ms, and only for a place whose kind can have an outline at all (`hasExtent`), since the only way to find one is the single-experience read and that route sits under `publicReadLimiter`. Cleared when the pointer leaves and when the selection goes; a place with no boundary draws nothing. The card carries the same fact in words, with the credit ODbL asks for (`ExtentLine`)
...
Map Mode builds a marker for every place — `buildExperienceMarkers()` in `components/experienceMarkers/buildMarkers.ts`, a pure function of the experiences, their locations, the expanded kind set and the folds this surface holds for this region (`collapsedExperienceIds`). It used to stop at 100 and show an on-map indicator saying so. The cap was removed because it silently disabled the list→map hover past it: the highlight resolves an experience through the marker set, and returns without a sound when it is absent, so in a 200-experience region half the rows hovered to nothing. The heatmap is what makes the full set affordable to render below its threshold.
...
(#910, [ADR-0061](../decisions/0061-the-catalogues-points-are-a-tile-layer-of-their-own.md)):
...
the layer reads `tile_experience_points` through Martin instead — the mechanism the region
polygons already use, with the reader's four questions written into the function itself,
because Martin serves it unauthenticated (`docs/security/SECURITY.md` § Known Gaps).

| | a region's markers | the world layer |
|---|---|---|
| where the features come from | `useRegionLocations`, one batch per region | `tile_experience_points`, one tile per z/x/y |
...
| the fold | per object, held per region (`useCollapsedExperiences`) | the whole map at once, a filter on the tile's `main` |
...
**The fold is one filter, and it reaches the heat.** The tile flags each object's *reader
position* — the place nearest its own published point, ADR-0028 decision 2, which is where a
region's folded pin already sits — and folding is `['has', 'main']` on the marker layer and
on the heatmap alike. It reaches the heat because that is where the distortion shows first:
...
- Curator on a place's row (Map mode's `LocationRow`, Discover's `PanelLocationRow`) -> a button named for the place, shown only for the row under the pointer, the one holding keyboard focus or the one lit from the map (opacity, not display, so it stays in the tab order — thirty rows do not carry thirty pencils), opens `PointPreviewDialog` on `PointCorrection` — the same dialog the review page and the object screen open, because a place is corrected wherever a curator is looking at one (#583). The way in is one function per object, bound in `ExperienceListItem` beside `handleCurate` and handed down unchanged, since the row is memoised (§ What a hover is allowed to re-render); the closure over the row is made inside the row, where it costs that row's render alone. A save invalidates the region batch, so the marker moves. A place outside the region gets the button too. The single place has no row on a card and is reached through the object screen (`CurationPlaces`)
...
- **Kind headers carry both numbers** while filtering — "World Heritage Sites (12 of 467)" — because one number cannot say both what is here and that the region holds more.
...
The list reads as groups and renders as one sequence. `flattenGroups()` in `ExperienceList/utils.ts` turns the kind groups and the experiences of the expanded ones into a flat `FlatRow[]`, and `useVirtualizer` from `@tanstack/react-virtual` mounts only the rows the scroll window covers. A collapsed group contributes its header alone, which is what `unmountOnExit` did before, arrived at differently.

**Which group a region opens on is the address's call when the address made one.** `initiallyExpandedGroup()` (same module) opens the group holding the card the address names, and the first group otherwise — once per region. It used to be the first group unconditionally, which meant a link to a card in any other kind landed on the right region with the card it named collapsed out of sight: the panel open, the title right, and nothing of the object on screen. A search result is that same link written by one click (#592, ADR-0042), so both were fixed by the same rule. A card the region does not hold falls back to the first group, which is the window before the address drops it.
...
Measured on Europe, whose UNESCO kind holds 467 rows, from selecting the region to the list settling — dev build, so `StrictMode` doubles the renders:
...
- **Kind headers are rows too.** The header of a group below the window is not in the document — anything looking for one has to scroll to it first.
...
with the per-row kind colour passed as a CSS variable (`--tyr-row-color`, a plain inline style,
...
- The picture is given a **fixed `height: 250`**, not a maximum, and it renders only once its bytes are here. Reserving before loading was worse than not reserving at all: when this was measured only 330 of 1604 experiences had a picture that resolved, so on four cards in five the box appeared and was taken away again (#557). Every picture a run writes is a Commons file since ADR-0043, and *Fix pictures* in the admin panel replaces the stored ones on a database that predates it, so the failure is the exception now; the rule stays, because a card that moves once is a card that moves.
