...
- **Region reads** — `requireVisibleWorldView` (`backend/src/middleware/worldViewVisibility.ts`) guards routes that take a `worldViewId` or a `regionId`; a declared route names the same check as its `scope`, read from its parsed input, and the registry runs it after the caller is known (ADR-0071) — the experience reads are declared that way. The middleware reads the id from a `worldViewId` or `regionId` route param; a declared route's `scope` reads it from any part of its parsed input — the region counts from their `worldViewId` query, an experience's locations from an optional `regionId` query filter (marked below). Admins bypass the check; everyone else gets **404**, never 403 — a 403 would confirm the world view exists, which is exactly what hiding it is meant to prevent. A hidden world view and a nonexistent one answer identically. A missing or malformed id never reaches the check: the route's schema refuses it with a 400 first. The optional `regionId` query filter is the one id that may be absent — an unfiltered read is legitimate, so the check passes it through rather than 404ing. The guarded routes (check against `worldViewRoutes.ts` / `experienceRoutes.ts` for the current set):
