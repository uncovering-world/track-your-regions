...
- **No writer computes over a region while it is flagged hand-drawn** (`is_custom_boundary`, #439). Each geometry writer holds that rule itself:
  ...
  - **Every geometry write asks again** (`WHERE … is_custom_boundary IS NOT TRUE`). This covers the core, `computeGroupGeom`, the stream's save, the single-member fast path and the reset. A union can run for minutes, and a boundary drawn by hand in that time is kept: the write matches no row, and the writer answers *not computed* rather than success. The reset answers 409 in that case.
  - **`recomputeRegionGeometry`** (`helpers.ts`) guards its own write with `is_custom_boundary IS NOT TRUE`.
  - **The SSE stream and the `/geometry/compute` endpoint** keep the drawing (`shortCircuitForCustomBoundary`, `handleCustomBoundary`) unless they are told to replace it. The endpoint's `force=true` does that, and so does, for both, a member that carries its own `custom_geom`: they clear the flag first and then compute, since the region is no longer hand-drawn.
...
Every call `frontend/src/api/worldViews.ts`, `frontend/src/api/regions.ts` and `frontend/src/api/geometry.ts` make answers through a schema in `backend/src/api/responses/worldViews.ts`, `responses/regions.ts` or `responses/geometry.ts` (ADR-0066), except the ones that answer 204 with no body: deleting a world view or a region, and setting a region's geometry. Three geometry endpoints have no caller on the web. The two region-geometry list reads, `…/regions/root/geometries` and `…/subregions/geometries`, go unused because the map draws Martin's tiles. They declare no answer, and whether they stay is #1006. The third is the unstreamed computation, `POST …/regions/:regionId/geometry/compute`, which the editor does not call because it computes through the stream. Its answer is declared all the same, as `SingleRegionComputed`: the region computed, the region's hand-drawn boundary kept, or nothing to merge.
