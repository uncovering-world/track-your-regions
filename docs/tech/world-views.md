...
Every call `frontend/src/api/worldViews.ts` and `frontend/src/api/regions.ts` make answers through a schema in `backend/src/api/responses/worldViews.ts` or `responses/regions.ts` (ADR-0066), except the ones that answer 204 with no body: deleting a world view or a region, and setting a region's geometry. The two region-geometry list reads, `…/regions/root/geometries` and `…/subregions/geometries`, have no caller on the web (the map draws Martin's tiles), so they declare no answer; whether they stay is #1006.
