...
- **A curator can correct it**, through `PATCH /api/experiences/:id/works/:treasureId/edit`, which
...
- `experience_curation_log`: the audit trail — named rather than counted, for the reason
  ...
  curator putting a work's makers, title or year right (ADR-0040). The list is
...
`curatedFields` as well, since the dialog it opens in corrects it (#583). A row is a card where *either* half holds
...
- **Rows are grouped by subject, and the object is only the first.** A run records what moved
  ...
  point* beside it where the stored row was found (#583), or a work with its picture — until a
  part has an address of its own (#575). The subject also carries the
...
counts over listed rows (`shared/ContentsList.tsx`; an unread point's name opens
`PointPreviewDialog`, where it can be corrected, #583) — and `ItemHeader` and `messageFor` come from `queueCard.tsx`, which exists so every page
