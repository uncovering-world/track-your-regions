...
- **`syncUtils.ts`** — Single-location write, delegating to `locationWriter.ts` (`upsertSingleLocation()`), and sync log CRUD (`createSyncLog()`, `updateSyncLog()`, and `annotateClosedSyncLog()` for the narrow status/`error_details` write a follow-up step needs). Used by all five services. It deletes nothing: the FK-ordered per-source cleanup that force sync used lived here and is gone with it.
...
both branches, `region-counts`, `GET /api/experiences?regionId=`, and the `regions[]` of both
...
| GET | `/api/experiences/by-region/:regionId/locations` | Batch: all locations for all experiences in region, grouped by `experience_id`. Each row carries `curated_fields`, the claims a curator holds on the place, through the hand-built row the response is assembled from — the first cut selected the column and dropped it there — so a place's row on the map's card can say *pin corrected* (#583). Supports `includeChildren` and `includeLost`, the latter because this batch has to follow the list: a row the list shows but this omits arrives with no markers and a confident `0/N in region`. Eliminates N+1 per-experience location fetches. Excludes a `pending` container or a `pending` location, unconditionally |
