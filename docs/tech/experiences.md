...
Every read below except `/search`, `/kinds`, `/:id/finds` and `/points` is declared `optional` (ADR-0071), because its answer is shaped by who asks — a curator's rejected rows, a curator's or admin's whole `regions[]`, a gated museum's unread treasures, a reader's own `is_new` — and its `revalidate` policy says so in the headers: `Cache-Control: private, no-cache` and `Vary: Authorization`, so a shared cache cannot store one caller's answer and serve it to the next, while the browser keeps its ETag round-trip (#597; the reasoning is in `docs/security/SECURITY.md` § Headers). The four `public` reads are told nothing about who is calling, so a handler that reads the caller does not compile on one: that, and not a walk of the router, is what keeps a caller-shaped read off a shared cache.
...
Every response `requireAuth` lets through here carries `Cache-Control: private, no-store` and `Vary: Authorization`, set by the middleware itself: the bodies are one traveller's own history, and `no-store` is what keeps them out of the browser's disk cache after sign-out (#710; the reasoning is in `docs/security/SECURITY.md` § Headers). A request that never reaches it is refused by the route's limiter at 429, which runs first and carries nothing of the caller's to keep; a request `requireAuth` lets through is parsed after it, so a malformed one answers 400 under the same `no-store`. Every route on the user router is declared `signed-in` (ADR-0071), which builds `requireAuth` into its chain.
