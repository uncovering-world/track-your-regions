...
- **A division's boundary** (`DivisionGeometry`) is a GeoJSON feature of the stored `MultiPolygon`, at the `detail` the caller asks for (#1010). `low` and `medium` read the stored simplifications (`geom_simplified_low`, `geom_simplified_medium`, falling back to the full shape), which is what the previews ask for: the coverage, overlap, smart-simplify and import-review dialogs, and the editor's division search and custom-subdivision preview. France is 2.9 million characters of GeoJSON in full and about 100 000 simplified. `high`, the default, is the full shape, since the editor's split and cut tools store what they cut from it.
