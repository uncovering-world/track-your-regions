...
| Stored picture urls | Two columns hold a url that later reaches an `<img src>`, and both are held to an allowlist on both sides. On the way in, the rule is declared once (`backend/src/types/urlSafety.ts`) and put to the URL parser rather than matched against the raw string: a link is an absolute http(s) url; a picture is that, or a path whose origin survives resolution (`/images/…`, for a file we host). A curator's `image_url`, `websiteUrl` and `wikipediaUrl` read it (#693), and so do a region's imported map and its candidates in an admin's import tree, the map-image pick and the vision-match url — in the link form, since no map is a path on our own origin (#694); each of those used to be `z.string().url()`, which is `new URL()` in a try/catch and accepts `javascript:` and `data:`. Where a picture is drawn, `extractImageUrl` / `toThumbnailUrl` (`frontend/src/utils/imageUrl.ts`) decide again, and keep only an absolute http(s) url on a trusted host or a path on our own origin — load-bearing rather than a second copy, because a sync writes `image_url` through no request schema and the rows already stored predate the write-path rule. `urlSafety.test.ts` pins the behaviour, that the protocol is named in no other backend module, and, by reading every frontend component, that no `src` — a JSX `src={…}` or an imperative `img.src = …`, looked at through one binding — names a stored picture raw or sizes one by hand |
| Stored link urls | The one stored link held to the rule on **both** sides is a region's source page, `region_import_state.source_url`, posted in an admin's import tree and by the review's add-child and rename routes (#703). On the way in, the three schemas read `isStorableHttpUrl` — an absolute http(s) url and nothing else, stored as the parser read it — where each used to be `z.string().url()`. Where the page is offered, `safeHref` (`frontend/src/utils/safeHref.ts`, lifted out of `ImageCreditLine`, which asks it for a credit's licence and details links) decides again, and a refused page gets no link: a glyph not drawn, a button disabled, a title in plain text. React neutralises a `javascript:` href of its own accord (the click throws) but says nothing about `data:` or `blob:` and nothing at all about `window.open`, which the CV match review handed the stored value; the decision is the app's rather than borrowed from the framework. `urlSafety.test.ts` pins the three schemas and, by reading every frontend module (`.ts` and `.tsx`) that carries a stored link (`sourceUrl`, a credit's `licenseUrl` / `detailsUrl`), that no dynamic `href={…}`, `window.open(…)` or `.href = …` assignment in such a module reaches the sink as anything but a `safeHref(…)` call or a url the file spells itself (a literal, or a template opening on `https://`) — `safeHref(x) \|\| x` counts as raw — looked at through one binding, with a name the file binds to nothing counting as raw, since the value reaches a link through a prop as often as directly. An experience's website and Wikipedia links (`metadata.website`, `metadata.wikipediaUrl`) are held on the way in only — the row above — and reach three hrefs as stored; see Known Gaps, #708 |
| ORM | Drizzle ORM + parameterized `pool.query()` |
| Headers | Helmet (CSP, X-Frame-Options, etc.), plus cache control on every response whose body is shaped by the caller: `optionalAuth` (`backend/src/middleware/auth.ts`) sends `Cache-Control: private, no-cache` and appends `Authorization` to `Vary` on each response it lets through, because Express's default ETag + `Vary: Origin` says nothing about who asked and would let a proxy, or a future CDN, store one caller's answer under the URL alone and hand it to the next — an admin's world view list with the unpublished ones in it, a curator's region list with its rejected rows, a reader's own `is_new`. The rule sits on the middleware that puts the identity on the request rather than on each route (#597, after #460 had put it on the listing alone), so it reaches the world view listing, the seven `optionalAuth` experience reads and every read behind `requireVisibleWorldView` — where an admin gets 200 for a hidden world view and everyone else 404 — and a read added tomorrow carries it by carrying `optionalAuth`; `backend/src/routes/callerShapedReads.test.ts` walks both routers to hold that. `private` is what forbids the shared cache. `no-cache` rather than `no-store` is deliberate: these bodies hold nothing the checklist classifies as sensitive (V14.2.1), and the browser's revalidation round-trip is worth keeping — a region's list, its locations and its geometry all answer `304` to a conditional request, some 865 kB gzipped for Europe that `no-store` would re-download on every reload — while the origin answers each revalidation for the *current* caller, so a stored variant is never served to the wrong one. That round-trip is unconditional for an anonymous reader and bounded for a signed-in one: `Vary: Authorization` keys whatever private cache does store it on the caller, which makes a stored variant selectable only under the token it was stored with, and the access token rotates every 15 minutes — so they get the `304` within one token's lifetime and a full body on the first read after each refresh. `res.vary` appends, where `setHeader('Vary', …)` would clobber the `Origin` CORS put there. `requireAuth` is the other half of the rule, with the other value (#710): every response it fronts — everything under `/api/users/me`, `GET /api/auth/me`, every read behind `requireAdmin` and `requireCurator` — carries `Cache-Control: private, no-store` and the same `Vary: Authorization`. Those bodies are the caller's own — a traveller's visited regions, experiences, locations and viewed works, a profile with its email, an admin's import tree — what V14.2.1 classifies as sensitive, and the exposure is the other cache: a shared one is already kept out by RFC 9111 § 3.5, since the access token travels in `Authorization`, but the browser's private cache stored the body under Express's defaults (an ETag, no freshness) and served it back on `304` after sign-out, on whatever machine the traveller had signed in from. `no-store` costs nothing there — an id list, a profile — where it would cost `optionalAuth`'s reads their round-trip. The header goes on ahead of the token check, so a 401 carries it too; a handler that needs something else sets it after the middleware and wins, since `setHeader` replaces. Four admin image endpoints do: the water crop and cluster preview (both through `sendDataUrlAsImage`) and the cluster highlight keep `private, max-age=300`, the image proxy `public, max-age=86400`. What the added `Vary` is worth to them differs by how each is fetched, and only one is fetched in the sense `Vary` means: `image-proxy` is a `fetch` carrying the header (`ImageOverlayDialog.tsx`, `useImageColorPicker.ts`), so `Vary: Authorization` now keys its `public` entry per token — an entry a shared cache was permitted to store and reuse across callers, `public` being RFC 9111 § 3.5's own exception. That keying costs more than it buys, the way it does on the geometry reads below: the picture depends on `?url=` alone and is identical for every caller, so what a shared cache would have handed on is a public Commons image, while the browser's own entry now expires with the token rather than after a day — an admin working through an import review re-proxies the same picture every 15 minutes, and each miss is another request to Wikimedia, whose politeness rules this repo otherwise keeps. It stays for the same reason the geometry reads keep theirs: a handler subtracting from a header another module set is a mechanism to introduce deliberately rather than in passing. The other three are `<img src>` URLs built by `withTokenQuery` (`frontend/src/api/admin/wvImportCvMatch.ts`) precisely because a browser attaches no `Authorization` to an image request: `Vary` selects nothing there, and what bounds their five minutes is the token sitting in the URL, which changes when it rotates. The three SSE streams keep `private, no-cache` the same way, through `markStreamBody` since the value and its eight lines of reasoning were the same in each, and the `private` there is doing the work § 3.5 does elsewhere: `EventSource` cannot send headers, so their token rides in the query string and the request carries no `Authorization` at all — which is also why replacing the middleware's value rather than adding to it has to keep `private` deliberately. So do the three division geometry reads (`controllers/division/divisionGeometry.ts`) and the geoshape proxy (`controllers/admin/wvImportLifecycleController.ts`, Wikimedia's boundary for a Wikidata id, re-requested on every preview open), through the shared `markPublicReferenceBody` (`middleware/cacheHeaders.ts`) — and for the reason that decides the whole rule: `/api/divisions` sits behind `requireAuth` + `requireAdmin` because the editor is its only caller, not because the answer is anyone's own — what it returns is GADM's boundaries at full resolution, 2.8 MB of GeoJSON for France and the whole world for `/root/geometries`, fetched again on every dialog open with no client cache in front of it. `no-store` there would trade a `304` for megabytes and protect nothing, and the same holds for the geoshape, whose body is Wikimedia's rather than anyone's own. Those reads keep the `Vary: Authorization` too, so they are bounded the way the `optionalAuth` ones are — the `304` within one token's lifetime, the full body on the first open after each refresh; a body identical to every caller gains nothing from that key, but a handler subtracting from a header another module set is a mechanism to introduce deliberately rather than in passing. One class sits ahead of the middleware, where it reaches four of five: the responses that hand the caller an access token (`markTokenResponse`, `middleware/cacheHeaders.ts`) — `/login`, `/verify-email`, `/refresh` and `/exchange-code`, plus `/change-password`, which is behind `requireAuth` and already carries the value, marked with them so the five read alike. All five are `POST`, which no browser stores, but RFC 6749 § 5.1 asks a token response for the header outright, so they state `private, no-store` — and the `Pragma: no-cache` that RFC asks for beside it, deprecated in a response by RFC 9111 § 5.4 and read by nothing here, so it states the intent for an auditor rather than changing behaviour — rather than inherit the guarantee from the method. The rule is kept in two places, each fitting what it can see. `no-restricted-syntax` in `backend/eslint.config.mjs` fails any `Cache-Control` written in the backend that drops `private`, and any it cannot find `private` in at all — both the setter and the object-entry shape, keyed on the method so a chain or a differently named response is covered, and neither reaching an outbound `fetch`'s request headers; the one deliberate exception, the image proxy's `public` value, carries its reason as a suppression on its own line, and `cacheControlLint.test.ts` pins the selectors themselves in both directions against the real config. What a linter cannot do is fire on a write that stopped existing, so `cacheOverrides.test.ts` holds that: the files that write the value inline still write it, counted per file, the four public-reference reads and the three streams still call their helper, counted per handler, and every response handing back an access token is paired with its mark. `auth.test.ts` holds the header on the 401 paths and on the wire beside an SSE handler's own; `callerShapedReads.test.ts` walks the user router so every route there carries the middleware |
...
- **A region's map url has a third reader the picture rule does not cover.**
  The stack row above names the two sides the rule holds — the schema on the
  way in, the `<img src>` where the map is drawn. The CV colour-match pipeline
  (`wvImportMatchPipeline.ts`, `runSourceMapPipeline`) is neither: it
  `fetch`es `region_map_url` from the server, follows redirects, and pushes
  the decoded bytes back to the admin as a debug image. The scheme is bounded
  to http(s) since #694; the host is not, where `image-proxy` on the same
  router refuses anything outside `*.wikimedia.org` / `*.wikipedia.org`. Only
  an admin can post an import tree, and the Wikivoyage path cannot reach it
  (`parser.ts` builds every map url from a hard-coded Commons prefix), so what
  is unbounded is what a JSON file may name. Tracked in #706.
- **An experience's website and Wikipedia links reach an `<a href>` raw.**
  `metadata.website` and `metadata.wikipediaUrl` are held to an absolute
  http(s) url when a curator writes them (#693) and to nothing when a sync
  does — the museum and landmark syncs take `website` from Wikidata, whose
  URL datatype admits no `javascript:`, and every stored value is http(s)
  today (measured 2026-08-28) — and three surfaces put them into an href as
  stored: `ExperienceDetailPanel.tsx`, `ExperienceExpandedDetails.tsx` and
  the curation card's `ObjectContext.tsx`. Nothing executes now, because
  React blocks a `javascript:` href of its own accord; but that decision is
  the framework's, not the app's, and the href guard in `urlSafety.test.ts`
  reads only the modules that carry a region's source page. Tracked in #708.
