...
- **The `requireAuth` reads that return a traveller's own history carry no anti-caching header.** `GET /api/users/me/visited-regions`, `…/visited-experiences`, `…/visited-experiences/ids`, `…/visited-locations/ids` and their siblings on `userRoutes.ts` return what V14.2.1 classifies as sensitive — visited regions and experiences — with Express's defaults only: a weak ETag and no `Cache-Control`. A shared cache is not the exposure (RFC 9111 § 3.5 forbids storing the answer to a request that carried `Authorization`, and the access token travels in that header); the browser's own cache is: the body sits in the disk cache after sign-out on a shared machine, served back on `304`, which ASVS 5.0 14.3.2 asks `no-store` to prevent. `optionalAuth` got its rule in #597 (the Headers row); `requireAuth` is the other half, with the other value — `no-store`, since these bodies are the caller's own data and small. Tracked in #710.
