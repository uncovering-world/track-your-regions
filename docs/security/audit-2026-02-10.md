...
**Superseded by #442**: that first fix qualified one arbitrary region of the
experience while the response still carried the rows for all of them. The check
is now a per-row filter over the caller's scope closure — see
`docs/tech/experiences.md` § Curation Guarantees. The description above is kept
as the record of what was applied in February, not of what the code does now.

### F-03: editExperience Passes Null regionId — LOW — FIXED
...
**Fix applied**: #450 removed the call rather than widening the signature.
`editExperience` no longer hands `checkCuratorScope` a region at all — it
resolves the experience's assignments against the caller's scope closure in one
query (`resolveEditScope`). That was the only caller that could pass `null`;
every remaining one takes a region from the request, guarded before use, so the
recommended signature change is moot.
