...
reason. This is what reaches Roman Forum. It is guarded by the conditions that
already guard missing detection — the run finished without errors and was not
cancelled — plus ADR-0022's floor that no run may empty a category. UNESCO does
not call it: UNESCO publishes a collection, and absence there is the ambiguous
kind ADR-0020 was written about.
