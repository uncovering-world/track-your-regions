...
**The rule is decided in two places and read everywhere else** (#674). `geometry_focus(geom)` in `db/init/01-schema.sql` measures what the database holds, and both focus triggers store its answer — a region's and a division's `focus_bbox` + `anchor_point` are columns, served by the list and read endpoints. `focusFromGeoJson()` (`frontend/src/utils/mapUtils.ts`) is the same rule for a shape that exists only in the client — a boundary being drawn, a cut, a combined selection. Frame with `smartFitBounds`, passing both the box and the anchor: `cameraForBounds` does not handle a `west > east` box on its own. Never re-derive: not `turf.bbox` (the extremes of the raw longitudes — anything over the dateline comes back as `[-180, …, 180]`), not an envelope test, not a threshold over points. `docs/tech/geometry-columns.md` § How a crossing region is told from a global one has the rest, GADM's overshoot past +180 included; `backend/src/db/regionFocusAntimeridian.test.ts` fails on a third decision. (Roughly a dozen `turf.bbox` calls still frame a camera through `map.fitBounds` directly rather than `smartFitBounds` — admin and editor surfaces, tracked in #672.)
