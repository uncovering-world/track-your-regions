...
- A reader predicate (is this row one a reader may see) is composed from `db/readerPredicates.ts` / `db/membership.ts`, never spelled, and `experiences` is written only by its writer modules — `db/experienceWriter.ts` for curator writes, whose `lockExperience` token every write under the object lock requires, plus the run's upsert, missing detection and picture repair (ADR-0069). The backend lint refuses both a spelled-out predicate and a write elsewhere
