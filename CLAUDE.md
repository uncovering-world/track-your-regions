...
- **experience_kinds** — what a traveller browses by: World Heritage Sites (id=1), Art Museums (id=2), Public Art & Monuments (id=3), Places of worship (id=4), under the ids of the sources that fill them; a reader-facing row carries its kind off its membership (`rowKindJoinSql` in `backend/src/db/membership.ts`, #819)
...
- **experience_sources** — the *sources*: UNESCO (id=1), Art Museums (id=2), Public Art & Monuments (id=3), Places of worship (id=4, Wikidata, gated on arrival, ADR-0052), each naming the kind it fills (`kind_id`). `display_priority` controls ordering (lower = first). Named `experience_categories` until migration 055 (#819); `experiences.source_id` names the source that brought the place and is the row's identity arbiter until #755
