...
```bash
npm run gates              # What this change asks for: every gate, run or skipped, with its reason (docs/tech/gates.md)
...
**A gate runs when, and only when, the inputs it checks have changed.** `scripts/gates.mjs` is the map from each gate to its inputs; `npm run gates` prints which gates the current change asks for and why the rest are skipped; `docs/tech/gates.md` has the map and the reasoning. Before every commit: `npm run check` (the fast gates the change asks for; `npm run check:all` forces every one), `npm run gates -- run test` (the unit lanes it asks for; `TEST_REPORT_LOCAL=1` keeps them on the host), and `/security-check`. Before the pull request opens, and again on the head the maintainer is asked to merge when a review wave since then touched their inputs: `npm run security:all` (the fast gates plus the slow Semgrep and Trivy scans the change asks for) and, when `npm run gates` lists them, `npm run test:e2e:smoke`, `npm run test:db` and `npm run perf:local` — a review-wave push owes the per-commit tier alone, and CI answers for the slow lanes on the pushed head (`/commit` § 8 holds the rule, #920). A gate the map skips was not run and did not need to be; a gate the host cannot run (the Python tooling guard) is a failure to report, not a skip. CI reads the same map per job, so a skipped job is a job whose inputs the pull request does not touch. (ADR-0062, #783.)

`npm run gates -- run stack` prints the before-push lanes with run/skip marks and runs none of them: they assume Docker and minutes of runtime, and they are typed by hand. The exception to the rule above is a deliberately layered sequence — a schema committed before its consumer, a test before its implementation. Those intermediate commits are expected not to build, so commit them with the gate red and get it green before the branch leaves your machine. The gate binds the branch, not every commit in it. See `docs/tech/development-guide.md` § "Granular Commits".

Secret detection is split and does not fully compose: GitHub native scanning + push protection read every file type but match known provider patterns only, while Semgrep `p/secrets` (CI) covers source but skips the types `.semgrepignore` drops (`*.yaml`, `*.yml`, `*.json`, `*.sql`, `*.md` — note `*.yml`, which is what `docker-compose.yml` and `.github/workflows/*` are). A generic credential in a config file is seen by neither — see `docs/security/SECURITY.md` § Known Gaps. Those layers, the DCO check and CodeQL (JS+Python, via GitHub's default-setup code scanning, configured in repo settings rather than as a workflow file) are outside the map and read every change.

The smoke lane stands up the isolated test stack and seeds its fixture automatically before running the Playwright smoke specs. It retries a spec once and reports one that then passes as **flaky** — named, with its trace kept, and green rather than red (#447) — while a service that never answers its readiness probe prints `docker compose ps` and its own last 200 log lines before the run stops; `docs/tech/development-guide.md` § Reading a smoke result tells a pass (retried specs included), a failed spec and a stack that never got ready apart.

The map lists `perf` for any change under `app`, and what it costs is minutes: `npm run perf:local` measures everything this machine can on its own data (the bundle-size budget, Lighthouse on the dev stack's production build with the real catalogue, the latency probe), and the change that most needs it is one touching what the browser loads or draws — a dependency, a route, a layout, a hot endpoint. CI's `Performance (Lighthouse)` job runs the fixture lane (`npm run perf`) on every pull request whose inputs ask for it, and the bundle-size budget (`npm run perf:size`) sits in CI's build job. Performance is measured on the **production build only** — `npm run dev:frontend:preview` serves it from the dev stack, and the dev server's numbers are not the product's; see `docs/tech/performance.md` for the budgets and the rule for moving them.
...
`/pr-create` also measures the branch's **review surface** (`npm run review:surface`) before it opens the PR: a branch is split before its first review round rather than after reviewers have read the whole surface, and one over the budget either gets split along a seam the scorecard names or records in the PR body why the surface is inherently one change. That one is a signal, not a gate — nothing fails because the budget was crossed. `docs/tech/review-surface.md` holds the baseline it was derived from and the rule for moving it.

The repo-wide Node lint tooling lives in the **root** `package.json` (`madge`, behind `lint:circular`; `markdownlint-cli2`, behind `lint:md` — one of the gates a prose-only change runs, `lint:md`, `lint:links` and `lint:pointers`, and the only one of those with an install to its name), so `npm run check` needs one install there on top of `backend/`, `frontend/` and `packages/shared/` (the rules both sides apply, ADR-0065 — its own lint, typecheck and knip need its devDependencies; the two consumers only need the directory to exist): `npm install` in the repository root, once. Its version comes from the tracked root `package-lock.json` — the tree CI's checks job installs with `npm ci` — rather than from the registry at run time (#490). `scripts/require-node-tools.sh` guards the gate the way `require-py-tools.sh` guards the Python ones: a missing install names the check that did not run instead of dying on `sh: madge: command not found`.

Python dev tooling lives in `cv-python/requirements-dev.txt` (ruff, mypy, pytest, pytest-cov, bandit, pip-audit). One-time setup: `npm run setup:py:dev` creates `cv-python/.venv` and installs both requirements files. The `*:py` npm scripts call `.venv/bin/<tool>` directly so they work without venv activation. CI covers both shapes across two jobs: the checks job builds the same venv with `npm run setup:py:dev`, and the Python test job installs into the runner's own interpreter with `actions/setup-python` + `pip install`. The venv is required locally by any change the map sends to a Python gate — `scripts/require-py-tools.sh` guards each `*:py` gate and fails outright when a tool is missing, rather than letting `sh: .venv/bin/ruff: No such file or directory` read as environment noise while the rest of the chain silently does not run. A branch that touches no Python is never asked for it (#783) — unless it touches repository-wide tooling, which reaches every gate — and a branch that is asked and cannot answer reports a failure rather than a skip. `setup:py:dev` needs `python3.12` by name, since CI pins 3.12 and cv-python ships on `python:3.12-slim`; where that binary is absent *and* no venv exists, the guard prints a container command that runs the same gates against the pinned interpreter. An existing venv keeps working — the pin binds building one, not using one.

## Task Board & Issues

Every open issue belongs on the org project board — https://github.com/orgs/uncovering-world/projects/2 — (placement is best-effort: `board.sh add` reports a miss, and `/issues` surfaces off-board issues) and carries GitHub-native metadata on the issue itself (full semantics in the board README): a **type** (Bug / Feature / Task, or **Epic** for an umbrella that is decomposed into sub-issues and never worked on directly), the org **issue fields** **Priority** (Urgent / High / Medium / Low), **Size** (Tiny → X-Large, ≈1–8 points; `board.sh` takes the literal option words), **Theme** (product area) and **AI fit** (Agent-ready — specced end-to-end for an autonomous session / Pair — product calls expected midway / Human-led — the maintainer leads: product decisions, manual testing, external accounts), a **parent** when it is one slice of an epic, **blocked-by** dependencies, and the **milestone** when it belongs to the next goal. The board's one field of its own is **Status** (🆕 New → 📋 Backlog → 🏗 In progress → 👀 In review → ✅ Done). `scripts/board.sh` adds issues and sets fields, Status and type by name; parents, blockers and milestones go through `gh issue edit`. Only Status and the placement on the board need the `project` token scope (`gh auth refresh -s project`; without it the fielded `board.sh add` still sets the fields and exits 2 for the missed placement, while the fieldless form has no fields to set and exits 1); everything else needs `repo` plus `read:org` (the default `gh auth login` grant — `board.sh` lists the org's fields and types to validate a value before it writes).

- **Picking work**: `/issues` lists the Backlog by Priority with type, hierarchy and blockers. Match Size to capacity and AI fit to the session's mode, prefer the open milestone, then take the highest Priority that fits — never an Epic (pick one of its sub-issues; an Epic with no slice list yet, or with a listed slice — wherever its body keeps the list — still without its ledger mark (`→ #N`, `→ merged into #N` or `→ dropped: …`), is a decompose-first item, and `/feature <number>` on it first reconciles the sub-issues the Epic already has onto that ledger, then produces or completes the remaining slices instead of code, marking each on the Epic as it is created — blocked or not, the slices carrying its blockers), never a blocked non-Epic. Move the issue to 🏗 In progress when starting (`/feature` and `/fix` do this), 👀 In review when the PR opens.
- **Sequencing frame**: make the product fillable first (catalogue completeness, curation loop), then user-facing (design, account area, performance), production last. The next goal is the open milestone; vision epics carry the Epic type — a map of intent, not the next sprint.
- **New issues** follow the `.github/ISSUE_TEMPLATE/` forms (which set the type; features and tasks: Description / Requirements / Additional Information; bugs: the full bug template; an Epic has no form — it uses the Feature sections, lists its intended slices under Requirements, and is created with `--type Epic`), are written for any developer — English, neutral voice, no personal names or quoted chat — ground claims in real data (named sites, measured numbers), carry area labels only (`front`, `back`, `security`, `process`, …), and get Priority, Size, Theme and AI fit set — best-effort; a slice of an epic is filed as its sub-issue, a "depends on" as a blocked-by. `/issue-upload` keeps a file ledger of what each created or adopted issue still owes — its board placement, for an adopted issue the hierarchy, labels and type the batch confirmed, and, for a slice of an epic, its `→ #N` mark on the Epic's list — and cleans a line up only once all of them have landed (a create whose file mark then failed is the one case the ledger cannot see, and the command's lookup of recently created issues, shown for adoption before the batch is confirmed, covers it); a missed placement surfaces through `/issues`, which reconciles the board against open issues; a missed mark on a slice the Epic already lists through `/feature`'s reconcile pass; a missed append of a *new* slice only through that file ledger, which is why `/issue-create`, having none, writes the mark before it reports. Triage of 🆕 New items: verify the premise against the codebase first, then set the type, Priority, Size, Theme and AI fit and move to 📋 Backlog.
...
- **experiences** — the *place* (ADR-0045 decision 4): UNESCO sites, museums, public art, places of worship — identity, name, location, picture, the visit. Multi-location via `experience_locations` table
- **experience_kinds** — what a traveller browses by: World Heritage Sites (id=1), Art Museums (id=2), Public Art & Monuments (id=3), Places of worship (id=4), Archaeology (id=5), under the ids of the sources that fill them; a reader-facing row carries its kind off its membership (`rowKindJoinSql` in `backend/src/db/membership.ts`, #819)
...
- **experience_sources** — the *sources*: UNESCO (id=1), Art Museums (id=2), Public Art & Monuments (id=3), Places of worship (id=4, Wikidata, gated on arrival, ADR-0052), Archaeology (id=5, Wikidata, gated on arrival, ADR-0058), each naming the kind it fills (`kind_id`). `display_priority` controls ordering (lower = first). Named `experience_categories` until migration 055 (#819); `experiences.source_id` names the source that brought the place and is the row's identity arbiter until #755
- **treasures** — Independently trackable items inside venues (artworks, artifacts). Globally unique by `external_id`. Linked to experiences via `experience_treasures` junction table (many-to-many). A link the source stops placing is marked (`missing_since`), never deleted, and only by a run that cleared the works coverage floor (ADR-0044); every reader-facing read of a museum's works carries `offeredLinkSql`. `artists` is a list, because a work often has more than one maker and which one the catalogue held used to be an accident of row order (ADR-0040). What an admitted venue holds is read from the venue's own statements as well as from the classes a work can carry (`museum/venueSide.ts`, #890) — the Pergamon Altar is typed `altar` and no class of find names it — and what the kind's rule refuses there is on the run's changeset with its classes, never marked
...
- A rule both sides apply — the picture hosts and file types, a held picture's credit pairing, when a moved coordinate is a move and when it matters, the label fold and store rule, the near-global threshold, the curation-log action vocabulary, the changeset equality, the whole-region ceiling, the user roles and the auth providers (`USER_ROLES`, `AUTH_PROVIDERS`), the statuses a sync run, an import and a match may carry and the moves a run's status may make (`@tyr/shared/runStatuses`, refused otherwise by `guard_run_status_move()`), the lifecycle vocabularies and the curation-state moves (`@tyr/shared/lifecycle`) — is declared once in `packages/shared` (`@tyr/shared/<module>`) and imported by both (ADR-0065). It ships source, has no dependencies and no Node or DOM types, and a module in it imports nothing, not even a sibling. Where the schema also states the rule, the package is held to it by a type in `backend/src/db/curationLogActions.test.ts`, never by reading a file. A rule that needs a runtime — Express, React, `pg`, Zod, `import.meta.env` — stays on its side
- A reader predicate (is this row one a reader may see) is composed from `db/readerPredicates.ts` / `db/membership.ts`, never spelled, and `experiences` and `experience_locations` are written only by their writer modules (ADR-0069): `db/experienceWriter.ts` and `controllers/experience/experienceLocationWriter.ts` for curator writes, whose writes under the object lock all require the object's `LockedExperience`, plus the run's upsert, location writer, missing detection and picture repair. The backend lint refuses both a spelled-out predicate and a write elsewhere
- Startup cleanup in backend `index.ts` marks orphaned `running` sync logs as `failed`
- All frontend API calls go through `authFetchJson()` (`frontend/src/api/fetchUtils.ts`) with in-memory JWT, or through `authFetchOptionalJson()` beside it for a read whose endpoint answers 204 for "there is none" (the geometry reads): `authFetchJson` reads a 204 as an empty list, which is truthy and has none of the answer's keys; refresh token lives in an httpOnly cookie. The one *deliberate* exception is `changePassword`, which builds its request by hand because its endpoint answers a wrong *current password* with 401 while `authFetchJson` reads every 401 as an expired token — see `docs/tech/authentication.md` § Password Security. Other hand-built authenticated calls exist (`getCurrentUser`, the `image-proxy` fetches in `useImageColorPicker` and `ImageOverlayDialog`) and record no reason: treat them as debt, not as precedent. Pre-auth calls (login, register, refresh) are not exceptions at all — they have no token to send. A call's answer is typed by the type `@tyr/shared/api` generates from its backend schema, re-exported from the call's module, and never declared in `frontend/src/api/`; a success body sent around `respond()` fails the backend's lint (ADR-0066)
...
### Geometry & Triggers
Five trigger functions fire on region geometry changes: `update_region_metadata()` (area — cleared with the geometry, #763 — and `uses_hull` on INSERT only), `update_region_focus_data()` (anchor_point, focus_bbox), `update_regions_geom_3857()` (the 3857 column and every simplified rung), `invalidate_parent_region_geometry()` (nulls the derived parent, ADR-0035) and `refresh_region_geom_pieces()` (the leaf's pieces placement reads, ADR-0054 — so a leaf's geometry write also pays for its `ST_Subdivide` cut, about a minute for the largest). A sixth, `invalidate_member_regions_geometry()`, fires on `region_members`: a member change clears its region's geometry in the same statement, so no member writer calls `invalidateRegionGeometry()` — that is for structural changes, which name their parents (ADR-0068). Hull services in `backend/src/services/hull/` handle dateline-crossing geometries.
...
**The rule is decided in two places and read everywhere else** (#674). `geometry_focus(geom)` in `db/init/01-schema.sql` measures what the database holds, and both focus triggers store its answer — a region's and a division's `focus_bbox` + `anchor_point` are columns, served by the list and read endpoints. `focusFromGeoJson()` (`frontend/src/utils/mapUtils.ts`) is the same rule for a shape that exists only in the client — a boundary being drawn, a cut, a combined selection. Frame a shape a surface holds with `frameGeoJson(map, geojson, opts)`, and a stored `focusBbox` with `smartFitBounds`, passing the anchor: `cameraForBounds` does not handle a `west > east` box on its own. Never re-derive: not `turf.bbox` or a `Math.min` over longitudes or a `LngLatBounds` handed to `fitBounds` (the extremes of the raw longitudes — anything over the dateline comes back as `[-180, …, 180]`), not an envelope test, not a threshold over points. `docs/tech/geometry-columns.md` § How a crossing region is told from a global one has the rest, GADM's overshoot past +180 included; `backend/src/db/regionFocusAntimeridian.test.ts` fails on a third decision and on any `fitBounds` outside `mapUtils.ts` (#672).

### Experience Images
**Every picture a run writes is a Wikimedia Commons file, and that is a licence rule before it is a technical one** (ADR-0043); a curator may also name a file we host, as an `/images/…` path. The World Heritage Centre's terms forbid copying *and* in-lining its photographs, so a UNESCO site's picture comes from Commons through Wikidata (P757 "World Heritage Site ID" → P18 "image", read through `p:P757/ps:P757` — `wdt:` sees only the best-ranked statement and misses Cologne Cathedral), matched by the site's own id and nothing looser; the portal is linked to (`metadata.website`), never drawn. Museums, landmarks and artworks were Commons already. The hosts a picture may come from are `PICTURE_HOSTS` in `@tyr/shared/pictures`, imported by both sides (ADR-0065); every writer of `image_url` — the sync upsert, the works writer and the picture repairs (`isCommonsPictureUrl`, the run's rule), a curator's edit and publishing a held proposal (`isDisplayablePictureUrl`, which adds the `/images/` path) — refuses anything else, and the admin panel's *Fix pictures* repairs what is stored. Nothing is downloaded: `/data/images` machinery exists and holds 0 rows. Thumbnails via `Special:FilePath/X.jpg?width=N` (CDN-cached sizes: 120, 250, 330, 500, 960, 1280px) — no third-party resizer. Wikimedia requires proper User-Agent — built for every outbound call by `userAgent()` (`backend/src/config/userAgent.ts`), never spelled at a call site, with the `bot` marker on the source runs and the import's own services (it names the traffic, not who clicked) and `USER_AGENT_CONTACT` for a deployment that is not this one (#864) — 429+Retry-After handling, 1.5s delay between downloads.

**A hosted picture carries a credit.** Most Commons files are CC BY / CC BY-SA, which of a page that merely *shows* a picture ask the one thing — that the author is named wherever the work appears; ShareAlike binds adaptations, which showing a photograph is not. The World Heritage portal's photographs are not drawn at all (ADR-0043), so no credit the product shows answers to its terms — every one comes from Commons' `extmetadata`. Credits are captured at sync time into `metadata.imageCredit` on **both** `experiences` and `treasures` (`imageCredit.ts`) and rendered by `ImageCreditLine`. Anything new that displays an experience's or a work's picture shows the credit with it — the curator screens behind `requireCurator` included, since working on the catalogue rather than publishing it does not change whose photograph it is. On a **dense** row or tile, pass `redundantWith={artists}`: Commons names the painter as the author of a photograph of a painting, so without it the line repeats a maker the row already shows — and the row names every one of them (ADR-0040), so the credit is redundant when it matches *any* (`creditAddsBeyond`; see `docs/tech/shared-frontend-patterns.md`). Hang the credit on the picture that is actually on screen — never render one beside an image that failed to load.

**An outline the catalogue draws carries a credit too.** A site's extent is OpenStreetMap's work under ODbL, and ADR-0059 decision 3 asks for "© OpenStreetMap contributors" wherever that data is shown — near it, and visible without interaction. `ExtentLine` is the one component that draws it, and it renders the extent and the credit together or renders nothing: a credit beside nothing credits nobody, which is the picture-credit rule one surface over. Where a change of the extent is *described* rather than drawn — the review card's `boundary` row (`fieldMeaning.tsx`), on a screen with no map — the sentence carries the credit itself, since a tooltip is not visible without interaction. The basemap's own attribution corner is not that credit — it answers for the tiles, and this answers for the outline drawn on top of them.
...
- A row's `curation_state` moves only as `CURATION_MOVES` (`@tyr/shared/lifecycle`) lists: a trigger refuses `verified` → `pending` and `pending` → `auto` and every other unlisted move, whatever writer tries it (ADR-0070)
...
| **Anything that adopts a source for a kind** (a new sync, a regional tier, a curator's list) | `docs/tech/filling-a-kind.md` | The world and regional tiers, the scorecard and the terms checks (ADR-0048); `docs/sources/README.md` — the register a source is a record in before it is code |
...
| **Opening a pull request** (how much review the branch asks for) | `docs/tech/development-guide.md` § Review Surface | `docs/tech/review-surface.md` — the baseline over merged PRs, the budget and the rule for moving it |
| **Anything that adds or changes a gate** (a lint, a test lane, a scan, a CI job) | `docs/tech/gates.md` | ADR-0062 — the rule, the input classes, how CI applies the map and what it does not reach |
