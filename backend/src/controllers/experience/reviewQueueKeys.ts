 ...
 * `facet_region`, `reviewQueueFacets.ts`) removes the walk, and the facets are
...
, scoped AS (
  ...
  )${f.regionCte}${regionRootsCte(USER_ID)}
, page_rows AS (
    ...
    LIMIT ${f.limit}
  )${facetsSql(f, USER_ID)}${resultSql(f, filters.sort)}`;
