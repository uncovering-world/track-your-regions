...
export async function getReviewQueue(req: AuthenticatedRequest, res: Response): Promise<void> {
  ...
  // The curation log is scope-filtered **per row**, not per experience, and the
  // conflict card's two subqueries read it (`reviewQueueConflicts.ts`), as do the
  // answered lists that name a curator — so they carry the same predicate
  // `getCurationLog` does.
  ...
  // object, and the conflict card, each in a module of their own
  // (`reviewQueueContents.ts`, `reviewQueueConflicts.ts`): they are the queue's
  // largest statements, and the handler draws them from the same fragments.
