...
import { getOpenAIClient, isOpenAIAvailable } from '../../services/ai/openaiShared.js';

// ---------------------------------------------------------------------------
// Use the shared OpenAI client (single source of truth) so the availability
// gate at the route layer reflects the same instance used here. A second
// per-controller singleton can drift out of sync — e.g., return 503 even
// when the shared client is configured.
// ---------------------------------------------------------------------------
...
// Bounded-repetition regex to prevent catastrophic backtracking on malformed AI output.
...
function parsePass1Response(pass1Content: string):
  | { ok: true; flaggedBranches: FlaggedBranch[]; observations: string }
  | { ok: false } {
  try {
    let jsonStr = pass1Content;
    const fenceMatch = JSON_FENCE_REGEX.exec(pass1Content);
    if (fenceMatch) jsonStr = fenceMatch[1];
    ...
    // Validate flaggedBranches — drop entries without a positive integer
    // regionId. queryTree() falls back to a full-tree query when given
    // undefined, which is wasteful AND can blow up the pass-2 prompt.
    const validBranches = Array.isArray(parsed.flaggedBranches)
      ? parsed.flaggedBranches.filter(
          b => b && typeof b.regionId === 'number' && Number.isInteger(b.regionId) && b.regionId > 0,
        )
      : [];
    return {
      ok: true,
      flaggedBranches: validBranches,
...
async function runPass2(
  ...
  const parsed = parseStructuredResponse(pass2Content);
  return {
    report: parsed.report,
    actions: parsed.actions,
    promptTokens: pass2Response.usage?.prompt_tokens ?? 0,
    completionTokens: pass2Response.usage?.completion_tokens ?? 0,
  };
}

async function runFullTreeReview(
  client: OpenAI,
  model: string,
  worldViewId: number,
  startTime: number,
  ...
  const pass1PromptTokens = pass1Response.usage?.prompt_tokens ?? 0;
  const pass1CompletionTokens = pass1Response.usage?.completion_tokens ?? 0;
  ...
  if (flaggedBranches.length === 0) {
    return {
      report: observations || 'No issues found. The hierarchy looks well-structured.',
      actions: [],
      promptTokens: pass1PromptTokens,
      completionTokens: pass1CompletionTokens,
      passes: 1,
    };
  }

  const pass2 = await runPass2(client, model, worldViewId, flaggedBranches, observations);
  return {
    report: pass2.report,
    actions: pass2.actions,
    promptTokens: pass1PromptTokens + pass2.promptTokens,
    completionTokens: pass1CompletionTokens + pass2.completionTokens,
    passes: 2,
  };
}
  ...
  try {
    ...
    res.json({
      ...
      stats: {
        ...
        inputTokens: review.promptTokens,
