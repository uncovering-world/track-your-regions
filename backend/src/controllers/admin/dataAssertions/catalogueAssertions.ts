...
export interface CatalogueAssertion {
   ...
   * accepted (ADR-0022 is why: a point a source dropped keeps the tick a
   * traveller earned).
   */
...
/**
 * A stored string, safe to put in a line of the report.
 *
 ...
 */
const text = (row: AssertionRow, key: string): string =>
  String(row[key] ?? '').replace(/[\u0000-\u001F\u007F-\u009F]/g, ' ');
/** `int4` comes back as a number, `count(*)` as a string. Both survive this. */
const count = (row: AssertionRow, key: string): number => Number(row[key] ?? 0);
/**
