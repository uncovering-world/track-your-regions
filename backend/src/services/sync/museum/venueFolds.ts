...
function applySameSpotFolds(venues: FoldCandidate[], folds: Record<string, Fold>): void {
  for (const a of venues) {
    for (const b of venues) {
      ...
      const aLoses = a.works !== b.works ? a.works < b.works : a.sitelinks < b.sitelinks;
      if (aLoses) folds[a.qid] = { into: b.qid, metres: Math.round(d), why: 'same spot, recorded twice' };
    }
  }
}
...
function breakFoldCycles(folds: Record<string, Fold>, byQid: Map<string, FoldCandidate>): void {
  const survivesOver = (a: string, b: string): boolean => {
    const x = byQid.get(a);
    const y = byQid.get(b);
    if (!x || !y) return a > b;
  ...
  for (const start of Object.keys(folds)) {
    ...
    let keeper = cycle[0];
    for (const q of cycle) if (survivesOver(q, keeper)) keeper = q;
    delete folds[keeper];
  }
}
