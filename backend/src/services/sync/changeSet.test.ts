...
/**
 * The equality a curation card reads by is the one this diff reads by:
 * `jsonEquals` from `@tyr/shared/equality`, whose four properties are stated
 * once, beside it (#570, ADR-0065). One case here holds the diff to it — a
 * metadata value compared by that equality and not by anything narrower.
 */
describe('the equality a curation card reads by', () => {
  it('compares a metadata value with the shared equality', () => {
    // Key order is not a change; a null key is not a change; array order is.
    const before = snapshot({ metadata: { imageCredit: { author: 'G. Brigas', license: '© UNESCO' }, criteria: ['(i)', '(ii)'] } });
    const reordered = snapshot({ metadata: { imageCredit: { license: '© UNESCO', author: 'G. Brigas' }, criteria: ['(i)', '(ii)'], website: null } });
    const resequenced = snapshot({ metadata: { imageCredit: { author: 'G. Brigas', license: '© UNESCO' }, criteria: ['(ii)', '(i)'] } });

    expect(computeChangeSet(before, reordered, [], WROTE).changeType).toBe('unchanged');
    const result = computeChangeSet(before, resequenced, [], WROTE);
    expect(result.changeType).toBe('updated');
