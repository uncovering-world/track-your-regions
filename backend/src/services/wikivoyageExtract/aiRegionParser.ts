...
export async function extractRegionsWithAI(
  ...
  // A transient AI failure (timeout, rate limit, model lookup) must not abort
  // the whole extraction run — the deterministic parser already handles
  // empty AI results downstream.
  let model: string;
  try {
