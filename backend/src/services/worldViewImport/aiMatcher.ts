...
/**
 * Extract the JSON payload from a model response that may be wrapped in a
 * fenced code block (``` or ```json). Uses linear scans (indexOf) rather
 * than a regex to avoid super-linear backtracking.
 */
function extractFencedJson(content: string): string {
  const FENCE = '```';
  const first = content.indexOf(FENCE);
...
Use additionalDivisions when a region maps to 2+ GADM divisions (e.g., Donbas → Donetsk + Luhansk oblasts). Leave it as [] for single-division matches.`;
  ...
  try {
    ...
    // Parse JSON response — extract fenced code block if the model wrapped
...
interface SingleRegionContext {
  row: { id: number; name: string; is_leaf: boolean; match_status: string; suggestions: MatchSuggestion[]; rejected_ids: number[]; ancestor_path: string };
  rejectedIds: Set<number>;
  assignedIds: Set<number>;
  existingSuggestions: MatchSuggestion[];
}

/** Load region row + already-assigned division IDs. */
async function loadSingleRegionContext(
  worldViewId: number,
  regionId: number,
): Promise<SingleRegionContext> {
  const result = await pool.query(`
    WITH RECURSIVE ancestors AS (
      SELECT id, name, parent_region_id, id AS leaf_id
...
/**
 * No-OpenAI fallback: use trigram similarity if its score is high enough and
 * the division is not already rejected/assigned. Throws if trigram is too
 * weak since the caller explicitly asked for AI matching.
 */
async function handleTrigramOnlyFallback(
  regionId: number,
  isLeaf: boolean,
  validTrigramMatch: { divisionId: number; name: string; path: string; similarity: number } | null,
  ...
  return {
    improved: false,
    suggestion,
    reasoning: `Trigram match (${Math.round(validTrigramMatch.similarity * 100)}% similarity)`,
    cost: 0,
  };
}

/** Hydrate a MatchSuggestion from an AI result by looking up the division in the DB. */
async function buildSuggestionFromAIResult(
  aiResult: AIMatchResult,
): Promise<MatchSuggestion | undefined> {
  if (!aiResult.divisionId) return undefined;
  ...
  if (divResult.rows.length === 0) return undefined;
...
export async function aiMatchSingleRegion(
  ...
  const ctx = await loadSingleRegionContext(worldViewId, regionId);
  const regionName = ctx.row.name;
  const isLeaf = ctx.row.is_leaf;

  // Phase 1: Try trigram similarity match (free, fast)
  const trigramMatch = await tryTrigramMatch(regionName);
  const validTrigramMatch = trigramMatch
    && !ctx.rejectedIds.has(trigramMatch.divisionId)
    && !ctx.assignedIds.has(trigramMatch.divisionId) ? trigramMatch : null;

  const mergedSuggestions = [...ctx.existingSuggestions];
  if (validTrigramMatch && !ctx.existingSuggestions.some(s => s.divisionId === validTrigramMatch.divisionId)) {
  ...
  // Phase 2: AI matching (if available); otherwise fall back to trigram
  if (!isOpenAIAvailable()) {
    return handleTrigramOnlyFallback(regionId, isLeaf, validTrigramMatch);
  }
  ...
  const leaf: UnresolvedLeaf = {
    id: ctx.row.id,
    name: regionName,
    ancestorPath: ctx.row.ancestor_path ?? '',
    matchStatus: ctx.row.match_status,
  ...
  const suggestion = await buildSuggestionFromAIResult(aiResult);
