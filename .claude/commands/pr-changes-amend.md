...
- Remind the user they need to **force-push**: `git push --force-with-lease` (sound as long as nothing refreshed `origin/<branch>` since the last look at the branch — no refspec-less `git fetch origin`, no bare `git pull`; see `/pr-create` § 8) — and to close the wave as `/pr-create` § 8 says: the thread replies, the `Review round <n> — dispositions` comment (§ 8 *Dispositions*) and the `/review` that asks for the round (that one alone is skipped on a PR labelled `review-on-push`, where the push is the ask; the replies and the round comment still go up there), in the order § 8 "The next round" states for the PR's label state
