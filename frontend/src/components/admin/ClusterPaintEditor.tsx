...
export default function ClusterPaintEditor({
  ...
  // --- Undo/redo ---
  // Optional `pathsOverride` lets callers snapshot a freshly-computed paths
  // array synchronously, without waiting for the React render cycle to update
  // pathsRef. Avoids the setTimeout(0) trick around setPaths.
  const saveSnapshot = useCallback((pathsOverride?: BorderPath[]) => {
    const cc = colorCanvasRef.current;
    const colorData = cc ? cc.getContext('2d')!.getImageData(0, 0, cc.width, cc.height) : null;
    const sourcePaths = pathsOverride ?? pathsRef.current;
    const snap: Snapshot = { paths: structuredClone(sourcePaths), colorData };
  ...
  }, [canvasSize.w, canvasSize.h, overlayImageUrl, saveSnapshot]);
  ...
  // Captured by handleEraserHit's functional updater so handleSvgMouseUp can
  // snapshot the final post-drag paths synchronously, without depending on
  // pathsRef.current being flushed by React between the last mousemove and mouseup.
  const lastErasedPathsRef = useRef<BorderPath[] | null>(null);
  ...
  const handleEraserHit = useCallback((px: number, py: number) => {
    setPaths(prev => {
      ...
      if (changed) {
        lastErasedPathsRef.current = next;
        return next;
      }
      return prev;
    });
  ...
  const finishPolyline = useCallback((close: boolean) => {
    ...
    setPaths(nextPaths);
    setPolyPoints([]);
    // Snapshot synchronously with the freshly-computed paths array — no need
    // to wait for React to flush the setPaths into pathsRef.
    saveSnapshot(nextPaths);
  }, [saveSnapshot]);
  ...
  const handleSvgMouseUp = useCallback(() => {
    if (eraserDragRef.current) {
      ...
      const finalPaths = lastErasedPathsRef.current;
      lastErasedPathsRef.current = null;
      // If the drag actually mutated paths, snapshot the final array
      // synchronously; otherwise nothing changed and there's nothing to save.
      if (finalPaths) saveSnapshot(finalPaths);
    }
  }, [saveSnapshot]);
  ...
  else if (tool === 'fill' || tool === 'line' || tool === 'eraser') wrapperCursor = 'crosshair';
