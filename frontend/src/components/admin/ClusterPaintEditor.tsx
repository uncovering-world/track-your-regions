...
import type { ClusterReviewCluster, ManualClusterResponse } from '../../api/adminWorldViewImport';
...
export default function ClusterPaintEditor({
  ...
  useEffect(() => {
    ...
    // Load processed image onto border canvas. crossOrigin must be set BEFORE
    // setting `src`, otherwise the canvas becomes tainted in cross-origin
    // deployments (frontend on :5173, backend on :3001) and `getImageData`
    // fails in saveSnapshot/flood-fill/undo.
    ...
    // brushSize is intentionally not in deps: changing it should NOT reload the
    // image or wipe history. The tool-sync effect below keeps Atrament's weight
    // in sync as the slider moves.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [canvasSize.w, canvasSize.h, sourceImageUrl, overlayImageUrl, saveSnapshot]);
