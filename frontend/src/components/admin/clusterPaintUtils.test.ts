...
describe('floodFillFromSource', () => {
  ...
  it('stops at BORDER_DRAW_COLOR pixels on source canvas (rgb(21,101,192))', () => {
    // Source has a drawn border at column 2 in BORDER_DRAW_COLOR (the brush/polygon color)
    const source = createPixelData(4, 1, [255, 255, 255, 255]);
    setPixel(source, 2, 0, 21, 101, 192, 255);
    const overlay = createPixelData(4, 1);
    // Tolerance 100 skips source-color check, so only the border-color stop applies
    floodFillFromSource(source, overlay, 0, 0, [0, 255, 0, 200], 100);
    expect(getPixel(overlay, 0, 0)).toEqual([0, 255, 0, 200]); // filled
    expect(getPixel(overlay, 1, 0)).toEqual([0, 255, 0, 200]); // filled
    expect(getPixel(overlay, 2, 0)).toEqual([0, 0, 0, 0]);     // border untouched on overlay
    expect(getPixel(overlay, 3, 0)).toEqual([0, 0, 0, 0]);     // beyond border, unfilled
  });
