 ...
 * card distinguishes them: an initial sync (creates, no updates), whose
 * non-zero Created tile would otherwise sit over "No changes recorded for this
 * run"; a run whose changeset write failed, which leaves the lost-changeset
 * marker; and a run that predates the feature entirely. The fourth case is a
 * record that is there and short — the insert goes in batches with no
 * transaction around them, so part of a run's changeset can land under the same
...
describe('changesetNote', () => {
  ...
  it('says the record is short when part of it landed', () => {
    ...
    // Read off `has_changeset` alone, the dialog would draw that list with no
    // note beside a Held tile saying 1,272.
  ...
  it('explains an initial sync that created rows but kept no record', () => {
    ...
    // total_updated would leave this one showing "Created: 1,247" above
  ...
  it('calls a lost changeset what it is on a run that only held', () => {
    ...
    // missing, 1272 held. Gating on the three older counters would read that
    // as "nothing happened" under a Held tile saying 1,272.
    render(<>{changesetNote(log({
      total_held: 1272,
      changeset_lost: true,
    }))}</>);
