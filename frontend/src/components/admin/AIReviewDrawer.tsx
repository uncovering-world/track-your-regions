...
/** Update a single action inside the active report without touching others. */
function updateActionInReport(
  prev: Map<string, StoredReport>,
  activeReviewKey: string,
  actionId: string,
  patch: Partial<ReviewAction>,
): Map<string, StoredReport> {
  const next = new Map(prev);
  const report = next.get(activeReviewKey);
  if (!report) return prev;
  const updatedActions = report.actions.map((a) => (a.id === actionId ? { ...a, ...patch } : a));
  next.set(activeReviewKey, { ...report, actions: updatedActions });
  return next;
}

/** Format the stats line at the bottom of the drawer. */
function formatStatsLine(stats: HierarchyReviewResult['stats'] | null): string {
  if (!stats) return '';
  const passesLabel = stats.passes > 1 ? 'passes' : 'pass';
  const tokenTotal = (stats.inputTokens + stats.outputTokens).toLocaleString();
  return `${stats.passes} ${passesLabel} · ${tokenTotal} tokens · $${stats.cost.toFixed(4)}`;
}
    ...
    >
                    ...
                    <Checkbox
                      ...
                      onChange={() => {
                        ...
                        setReviewReports((prev) =>
                          updateActionInReport(prev, activeReviewKey, action.id, { completed: !action.completed }),
                        );
                      }}
                      ...
                      {action.choices && action.choices.length > 0 && !action.completed && (
                        <RadioGroup
                          ...
                          onChange={(e) => {
                            ...
                            setReviewReports((prev) =>
                              updateActionInReport(prev, activeReviewKey, action.id, { selectedChoice: newChoice }),
                            );
                          }}
                          sx={{ ml: 1 }}
                        >
      ...
      <Box sx={{ px: 2, py: 1, borderTop: 1, borderColor: 'divider', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography variant="caption" color="text.secondary">
          {formatStatsLine(activeReport?.stats ?? null)}
          {activeReport && activeReport.actions.length > 0 && (
