...
function suggestionIcon(type: RuleReviewResult['suggestions'][number]['type']) {
  if (type === 'merge') return <MergeIcon fontSize="small" />;
  if (type === 'contradiction') return <ConflictIcon fontSize="small" color="warning" />;
  return <ObsoleteIcon fontSize="small" color="error" />;
}
  ...
  const applyMutation = useMutation({
    ...
    onSuccess: (_data, appliedSuggestion) => {
      ...
      setReviewResult(prev => {
        ...
        // (e.g. two merge groups that both keep rule #1), so identity removes only the applied one.
        const remaining = prev.suggestions.filter(s => s !== appliedSuggestion);
