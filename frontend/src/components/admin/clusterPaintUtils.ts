...
export function overlayToPixelLabels(
  ...
  // Validate palette label range up-front. Out-of-range writes silently wrap
  // when assigned to Uint8Array, which would corrupt the label mapping.
  for (const { label } of palette) {
    if (!Number.isInteger(label) || label < 0 || label > 255) {
      throw new Error(`Palette label out of range [0,255]: ${label}`);
    }
  }
  ...
  // Sized to the full uint8 range so any valid palette label can be addressed.
  const colorCentroids: Array<[number, number, number] | null> = new Array(256).fill(null);
