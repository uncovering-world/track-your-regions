...
export function AIDivisionList({
  ...
  // Icon selection for a suggestion — flattens what would otherwise be a nested JSX ternary.
  type SuggestionEntry = NonNullable<ReturnType<typeof suggestions.get>>;
  const renderSuggestionIcon = (data: SuggestionEntry | undefined) => {
    if (data?.loading) return <CircularProgress size={20} />;
    ...
    if (s) {
      return (
        <Tooltip title={`${s.confidence} confidence`}>
          <HelpOutlineIcon color={s.confidence === 'medium' ? 'warning' : 'disabled'} />
        </Tooltip>
      );
    }
    return null;
  ...
  const escalationLabel = (level: string | undefined): string => {
    if (level === 'reasoning_search') return '🔍 Search';
    if (level === 'reasoning') return '🧠 Reasoning';
    return '⚡ Fast';
  };
  const escalationColor = (level: string | undefined): 'info' | 'warning' | 'default' => {
    if (level === 'reasoning_search') return 'info';
    if (level === 'reasoning') return 'warning';
    return 'default';
  };
            ...
            <Box key={divisionKey}>
              ...
              >
                <ListItemIcon sx={{ minWidth: 40 }}>
                  {renderSuggestionIcon(suggestionData)}
                </ListItemIcon>
              ...
              <Collapse in={isExpanded && !!suggestionData?.suggestion}>
                <Box sx={{ px: 3, py: 1.5, bgcolor: 'grey.50' }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mb: 1, alignItems: 'center' }}>
                    <Chip
                      label={escalationLabel(suggestionData?.suggestion?.escalationLevel)}
                      size="small"
                      variant="outlined"
                      color={escalationColor(suggestionData?.suggestion?.escalationLevel)}
                    />
