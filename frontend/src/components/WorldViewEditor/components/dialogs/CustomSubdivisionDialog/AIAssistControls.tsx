...
function getAskAIButtonLabel(
  ...
  if (divisionsToProcessCount === 0) return 'All have suggestions';
  const suffix = forceReprocess ? ' all' : ' remaining';
  return `Ask AI (${divisionsToProcessCount}${suffix})`;
}
    ...
    <>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          >
            {getAskAIButtonLabel(batchProcessing, divisionsToProcessCount, forceReprocess)}
          </Button>
