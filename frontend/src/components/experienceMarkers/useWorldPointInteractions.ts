 ...
 * already fetched, picture and credit included; a world pin comes out of a
 * vector tile, and carrying `image_url` in it costs 90 kB on the densest tile
 * measured — against 145 kB for everything else on it — for a thumbnail on a
 * hover card. So the card names the object, the place and the kind, and shows
 * no picture rather than a broken one. Nothing here renders a credit, which is
...
/** What a tile feature says about the place under the pointer. */
export interface WorldPoint {
  experienceId: number;
  /** The place itself, which the tile carries from the labelling zoom. */
...
export interface WorldPointInteractionsParams {
  ...
  /** The kind's own name, for the card's second line; the tile carries only its id. */
...
function pointOf(feature: maplibregl.MapGeoJSONFeature): WorldPoint | null {
  const experienceId = feature.properties?.experienceId as number | undefined;
  if (experienceId == null) return null;
  return {
    experienceId,
    // A property, not `feature.id`: `tile_experience_points` passes no
    // `feature_id_name` to `ST_AsMVT` — it carries the place's id as
    // `locationId` instead, so that the id is in the properties where the
    // region sources keep theirs out of them.
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: maplibregl.MapMouseEvent) => {
      ...
      setHoverPreview({
        ...
        // chip, which is silent without a count — and the tile carries none.
